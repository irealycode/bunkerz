...
function Subd() {
    ...
    const get_store = async() =>{
        ...
        if (res.data != "505" && res.data != "no.") {
            ...
            console.log('ok',res.data.products)
  ...
  return (
    <div style={{position:'absolute',width:width,height:height,right:0,overflowY:'scroll',overflowX:'hidden',scrollbarWidth: 'none', msOverflowStyle:'none'}} >

        <DefaultT  banners={Banners} bannerTexts={bannerTexts} bannerImgs={bannerImgs} bannerDivs={bannerDivs} bannerProducts={products} storeProducts={storeProducts} bannerButtons={bannerButtons}   />
