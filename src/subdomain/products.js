...
function Products() {
    ...
    const delete_product = async() =>{
        for (let xxt = 0; xxt < productsSelect.length; xxt++) {
            if (productsSelect[xxt]) {
                const res = await axios.post(`http://${host}:4242/deleteproduct`,{'sid':sid,'jid':jid,'pid':products[xxt].pid})
                get_store()
                // console.log(res.data)
            }
        ...
        <div style={{height:'100%',width:'100%',display:'flex',justifyContent:'start',alignItems:'center',position:'relative',flexDirection:'column',overflow:'scroll',overflowX:'hidden'}} >
                <div style={{width:'50%',marginTop:50,position:'relative'}} >
                    ...
                    <div style={{position:'absolute',top:0,right:10}} >
                        <button onClick={()=>SetAddProduct(false)} style={{cursor:'pointer',height:44,border:'2px solid rgba(0,0,0,0.3)',borderRadius:5,backgroundColor:'rgba(0,0,0,0.2)',fontFamily:'mc',fontSize:20,overflow:'hidden',color:'gray',marginLeft:10}} >Discard</button>
                        <button onClick={async()=>{SetSave(true);await delay(200);SetSave(false);SetAddProduct(false);get_store()}} style={{cursor:'pointer',height:44,border:'2px solid rgba(0,0,0,0.3)',borderRadius:5,backgroundColor:'#FF9900',fontFamily:'mc',fontSize:20,overflow:'hidden',color:'white',marginLeft:10}} >Save</button>
                    </div>
                </div>
        ...
        return (
            <div style={{height:'100%',width:'100%',display:'flex',justifyContent:'start',alignItems:'center',position:'relative',flexDirection:'column',overflowY:'scroll'}} >
                <div style={{width:'90%',marginTop:50,position:'relative'}} >
                    ...
                    <button onClick={()=>SetAddProduct(true)} style={{cursor:'pointer',height:44,border:'2px solid rgba(0,0,0,0.3)',borderRadius:5,backgroundColor:'#FF9900',fontFamily:'mc',fontSize:20,overflow:'hidden',color:'white',marginLeft:10,position:'absolute',right:0,top:0}} >Add product</button>
                </div>
                <div style={{width:'90%',marginBottom:50,borderRadius:5,backgroundColor:'white',border:'1px solid rgba(0,0,0,0.2)',marginTop:30,display:'flex',alignItems:'start',justifyContent:'center',flexDirection:'column'}} >
                    <div style={{display:'flex',alignItems:'start',justifyContent:'center',flexDirection:'row',marginTop:10,marginBottom:10,width:'100%'}} >
                        ...
                        <button style={{cursor:'pointer',width:'10%',height:44,border:'2px solid rgba(0,0,0,0.3)',borderRadius:5,fontFamily:'mc',fontSize:20,overflow:'hidden',color:'rgba(0,0,0,0.8)',marginLeft:10}} >⇅ Sort</button>
                    </div>
...

                {
                    ...
                    <button onClick={()=>editproduct()} style={{cursor:'pointer',height:44,border:'2px solid rgba(0,0,0,0.3)',borderRight:0,fontFamily:'mc',fontSize:18,overflow:'hidden',color:'rgba(0,0,0,0.8)',paddingLeft:15,paddingRight:15}} >Edit product</button>
                    <div style={{position:'relative'}} >
                        ...
                        <div className='text' style={{position:'absolute',maxHeight:showOptions?200:0,backgroundColor:'white',borderTop:0,width:'100%',zIndex:2,overflow:'hidden'}} >
                            <button onClick={()=>deactivate()} style={{cursor:'pointer',height:44,border:'2px solid rgba(0,0,0,0.3)',width:'100%',borderTop:0,fontFamily:'mc',fontSize:18,overflow:'hidden',color:'rgba(0,0,0,0.8)',paddingLeft:15,paddingRight:15}} >In/Activate</button>
                            <button onClick={()=>delete_product()} style={{cursor:'pointer',height:44,border:'2px solid rgba(0,0,0,0.3)',width:'100%',borderTop:0,fontFamily:'mc',fontSize:18,overflow:'hidden',color:'red',paddingLeft:15,paddingRight:15,borderBottomLeftRadius:5,borderBottomRightRadius:5}} >Delete product</button>
                        </div>
                    </div>
                    ...
                    products.map((product,xy)=>{
                        ...
                        if (product.title.toLowerCase().startsWith(search.toLowerCase())) {
                            return(
                                <div key={xy} style={{width:'100%',alignSelf:'center',position:'relative',height:100,borderTop:'1px solid rgba(0,0,0,0.2)',borderLeft:0,borderRight:0,overflow:'hidden',display:'flex',alignItems:'center'}}>
                                    <div onClick={()=>selectP(xy)} style={{cursor:'pointer',width:15,height:15,border:'2px solid rgba(0,0,0,0.3)',borderRadius:10,display:'flex',alignItems:'center',justifyContent:'center',marginLeft:10}} ><div style={{width:'100%',height:'100%',borderRadius:10,backgroundColor:productsSelect[xy]?'#ff9900':'rgba(0,0,0,0)'}} ></div></div>
                                    ...
                                    <div style={{position:'absolute',right:'45%',display:'flex',alignItems:'center',justifyContent:'center'}} >
                                        <p style={{fontFamily:'mb',fontSize:18,color:'#ff9900',marginRight:5,backgroundColor:'rgba(255, 153, 0,0.2)'}} >{product.qts?product.qts:'0'}</p>
                                        <p style={{fontFamily:'mc',fontSize:20}} >in stock</p>
    
                                    </div>
                                    {product.active==false?
                                    <div style={{position:'absolute',left:'85%',padding:6,paddingLeft:12,paddingRight:12,borderRadius:30,marginLeft:10,backgroundColor:'rgba(51, 227, 2,0.5)',display:'flex',flexDirection:'row',justifyContent:'center',alignItems:'center'}} >
                                        <div style={{width:15,height:15,borderRadius:15,backgroundColor:'#2ca30b',marginRight:6,overflow:'hidden',position:'relative'}} ></div>
                                        <p style={{margin:0,fontFamily:'mb',fontSize:18,color:'rgba(0,0,0,0.8)',alignSelf:'start'}} >active</p>
                                    </div>:
                                    <div style={{position:'absolute',left:'85%',padding:6,paddingLeft:12,paddingRight:12,borderRadius:30,marginLeft:10,backgroundColor:'rgba(0, 0, 0,0.1)',display:'flex',flexDirection:'row',justifyContent:'center',alignItems:'center'}} >
                                        <div style={{width:15,height:15,borderRadius:15,backgroundColor:'gray',marginRight:6}} ></div>
                                        <p style={{margin:0,fontFamily:'mb',fontSize:18,color:'rgba(0,0,0,0.8)',alignSelf:'start'}} >inactive</p>
                                    </div>}

                                    
                                    {/* <div style={{position:'absolute',right:20,display:'flex',alignItems:'center',justifyContent:'center'}} >
                                        <div style={{width:10,height:10,borderRadius:10,backgroundColor:product.active==false?'#2ca30b':'gray'}}></div>
